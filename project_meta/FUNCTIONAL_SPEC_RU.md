...
- **Analyze:** Детекция регионов интереса (заголовки, текст) через `EnhancedDetector` (ранее `ContrastDetector`).
- **Score:** Семантическое ранжирование блоков через `BlockPrioritizer` на основе типа контента и позиции.
...
**В ПРОЦЕССЕ (Beta)**
- **Content-Aware Analysis:** Оценка важности блоков через плотность граней и вариативность цвета (Edge Density & Color Variance). Позволяет отличать текст от графики.
- **Semantic Scoring:** Приоритизация типов контента. Заголовки (Headers) и диаграммы получают более высокий вес, чем основной текст или футеры.
- **Positioning Intelligence:** Автоматическое распознавание элементов оформления (Header/Footer) на основе их вертикальных координат.
- **Trajectory Optimizer (PLANNED):** Умная сортировка ROIs для минимизации лишних перемещений камеры.
- **Adaptive Dynamics (PLANNED):** Динамическая настройка скоростей и времени задержки.
