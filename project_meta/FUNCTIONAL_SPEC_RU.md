...
**Версия:** 0.9.2 (Progress Indicator & Stats Fix)
**Дата:** 2026-02-23
...
- **Smart Zoom (Auto-Plan):** Автоматическая генерация сценариев на основе анализа содержимого слайдов. Режим `auto` автоматически выбирает OCR для текстовых PDF и Contrast для изображений.
...
- **Безопасное микширование (Audio Protection):** 100% стабильность при использовании фоновой музыки любой длительности. Алгоритм ограничивает `apad` длительностью видео, предотвращая бесконечные циклы рендеринга.
- **Совместимость (Native Mac Support):** Видео оптимизированы под QuickTime Player и системные фреймворки macOS за счет использования стандарта `yuv420p` и флага `faststart` (перемещение метаданных `moov` в начало файла).
- **Стабильность памяти (OOM Protection):** Стабильное потребление памяти при обработке длинных видео, отсутствие скачков нагрузки на GC и повышение общей отзывчивости системы. 100% стабильность работы на любых конфигурациях. Автоматическое масштабирование количества параллельных воркеров в зависимости от разрешения видео и доступной памяти.
- **Статическая типизация времени:** Все расчеты времени выравниваются по границам кадров (FPS) для исключения джиттера и проблем со склейкой.

## 3. Архитектура и Оптимизации

### 3.1. Аппаратное ускорение
Приложение автоматически выбирает лучший доступный энкодер:
- **macOS:** `h264_videotoolbox` (Apple Silicon / Intel). Форсирует стандарт `yuv420p` для максимальной совместимости.
- **NVIDIA:** `h264_nvenc`.
...
- **Render Caching:** Постоянное кэширование отрендеренных страниц на диске. Пропускает этап рендеринга для неизменных PDF-файлов.
- **Memory Budgeting:** Автоматическое управление количеством воркеров и потреблением RAM для предотвращения Out-Of-Memory ошибок.
...
- **Analyze:** Детекция регионов интереса через `EnhancedDetector` (границы) или `OCRDetector` (структурный текст из MuPDF). Поддержка гибкого парсинга HTML-координат и динамического DPI-масштабирования.
- **Score:** Семантическое ранжирование блоков через `SemanticScorer` на основе типа контента (заголовки, графики) и их позиции.
- **Plan:** Генерация маршрута движения камеры с помощью `TrajectoryOptimizer`, минимизирующего лишние перелеты, с сохранением фокуса на важных деталях.
...
| `-black-screen-duration` | Длительность черного экрана (интро/аутро) | `2.0` |
| `-black-screen-transition`| Тип перехода для черного экрана | как в `-transition` |
...
| `-qrurl` | URL для генерации QR-кода (белый на прозрачном фоне) | `bit.ly/4e0ngKK` |
...
| `-trace` | Режим трассировки: отрисовка направления движения и точек остановок | `false` |
...
| Флаг | Описание | По умолчанию |
|------|----------|--------------|
| `-analyze-mode` | Режим анализа (`auto`, `contrast`, `ocr`, `enhanced`) | `auto` |
...
### 5.3. Совместимость и Стандарты
- **Faststart Optimization:** Использование `-movflags +faststart` позволяет видео начинать воспроизведение сразу после открытия (или начала загрузки в браузере), не дожидаясь полной загрузки файла.
- **Color Space Standard:** Принудительное использование `yuv420p` во всех пайплайнах (включая аппаратные) гарантирует корректное отображение цветов в QuickTime и на мобильных устройствах.
...
**АКТИВНАЯ ФАЗА:**
- **Content-Aware Analysis:** Оценка важности блоков через плотность граней и вариативность цвета (Edge Density & Color Variance).
- **Semantic Scoring:** Приоритизация типов контента. Заголовки (Headers) и диаграммы получают более высокий вес (до 1.0), чем фоновый текст или футеры.
- **Trajectory Optimizer:** Умная сортировка найденных регионов с помощью жадного алгоритма, который комбинирует значимость блока (Priority) и физическое расстояние между ними (Distance Weight). Обеспечивает плавные пролеты камеры без хаотичных "прыжков" через всю страницу.
- **Adaptive Dwell Time:** Динамическая настройка времени задержки (stay duration) на блоках на основе их типа (диаграммы, текст) и визуальной плотности.

**В РАЗРАБОТКЕ:**
- **Path Smoothing (PLANNED):** Сглаживание траекторий (сплайны) и физика инерции.